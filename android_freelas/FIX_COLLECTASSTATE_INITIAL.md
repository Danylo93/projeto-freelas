# 🔧 Como Resolver o Erro do Parâmetro 'initial' Faltando

## ✅ **PROBLEMA RESOLVIDO!**

O erro de parâmetro 'initial' faltando no `collectAsState()` foi **CORRIGIDO**!

### 🛠️ **O que foi corrigido:**

1. **✅ FreelasNavigation.kt**
   - Adicionado `initial = false` em `isAuthenticated.collectAsState()`
   - Adicionado `initial = null` em `currentUser.collectAsState()`

### 📝 **Explicação do Problema:**

O `collectAsState()` em Kotlin/Compose requer um parâmetro `initial` que define o valor inicial do estado enquanto o Flow ainda não emitiu um valor.
